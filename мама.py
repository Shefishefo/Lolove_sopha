...
@loader.tds
class ILMAMod(loader.Module):
	"""Famous TikTok hearts animation implemented in FTG"""
	strings = {
		"name": "LoveMagics"
	}	
	
	async def ilmacmd(self, message: "telethon.tl.types.Message") -> None:
		"""This famous TikTok animation..."""
		if not message.out:
			message = await message.respond('ilma')
		...
		for i in range(8):
			...
			await sleep(0.3)
		for i in ["Л", "Люблю", "Люблю вас", "Люблю вас, мам✨)))!"]:
